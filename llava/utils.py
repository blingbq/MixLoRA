...
EVAL_TASKS = ['text_vqa', 'visual_spatial_reasoning', 'pope_rand', 'cifar_100',
              'cifar_10', 'mnist', 'snli_ve_classification', 'pope_adv', 'pope_pop', 'sentiment','union']
...
def get_task_def(task):
    ...
    # --------------- llava tasks -----------------
        ...
        instructs = [
            f"""Generate a comment based on the post and image in distinct sentiment categories."""
        ]
    elif task == 'union':
        instructs = [
            f"""Generate a comment based on the post and image in distinct sentiment and stance categories."""
        ]
...
def get_taks_id(task_name):
    task_id_dict = {'GQA': 0, 'VQAv2': 1, 'visualgenome_vqa': 2, 'ok_vqa': 3, 'image_quality': 4, 'VQA_attribute': 5, 'image_text_selection': 6, 'VQA_counting': 7, 'wikihow_immediate_next_step_selection': 8, 'VQA_scene_recognition': 9, 'VQA_object_presence': 10, 'wikihow_next_step': 11, 'VQA_positional_reasoning': 12, 'wikihow_text_image_step_order': 13, 'question_image_match': 14,
                    'ITM': 15, 'VQA': 16, 'VQA_object_recognition': 17, 'wikihow_image_text_step_order': 18, 'VQA_color': 19, 'VQA_sport_recognition': 20, 'multimodal_factual_checking': 21, 'VQA_activity_recognition': 22, 'image_caption': 23, 'VQA_sentiment_understanding': 24, 'VQA_utility_affordance': 25, 'llava_conversation': 26, 'llava_detailed_caption': 27, 'llava_complex_reasoning': 28, 'sentiment': 29,'union':30}
